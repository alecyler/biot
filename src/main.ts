...
let lastUiRefresh = 0;
let lastSelectedBiotId: string | null = null;
...
saveSelectedBiotBtn.addEventListener("click", () => {
  const selected = world.getBiotById(selectedBiotId);
  if (!selected) return;
  const capturedName = `Captured ${selected.id}`;
  saveFavoriteBlueprint(capturedName, selected.segments);
  loadBiotIntoBuilder(selected.segments, capturedName);
  world.setCuckooBlueprints(getSavedBlueprintSegments());
  saveSelectedBiotBtn.textContent = "Loaded into builder";
  window.setTimeout(() => {
...
function updateUi(now: number, selectedBiot: ReturnType<typeof world.getBiotById>): void {
  if (now - lastUiRefresh < uiRefreshMs && selectedBiotId === lastSelectedBiotId) return;
  lastUiRefresh = now;
  lastSelectedBiotId = selectedBiotId;
...
function frame(now: number): void {
  ...
  const selectedBiot = world.getBiotById(selectedBiotId);
  if (!selectedBiot) {
    selectedBiotId = null;
  }

  const env = world.getEnvironmentState();
  if (advanced || env.version !== lastRenderedVersion || selectedBiotId !== lastSelectedBiotId) {
